...
class Servo:
    def __init__(self, pin):
        ...
        self.name = str(pin)
    def __init__(self, pin,start=MID):
        self.pwm = PWM(Pin(pin, Pin.OUT))
        self.pwm.freq(50)
    ...
    def center(self):
        ...
        if self.current_pos > self.center_pos:
            print("inside if")
            while ((self.current_pos - int(STEP)) > self.center_pos):
                print("inside while")
                self.current_pos = self.current_pos - int(STEP)
                self.pwm.duty_ns(self.current_pos)
                sleep(.3)
            self.current_pos = self.center_pos
            self.pwm.duty_ns(self.current_pos)
        elif self.current_pos < self.center_pos:
            print("inside else")
            while ((self.current_pos + int(STEP)) < self.center_pos):
                print("inside while")
                self.current_pos = self.current_pos + int(STEP)
